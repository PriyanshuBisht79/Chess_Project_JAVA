...
//		pieces.add(new Queen(WHITE,4,4));
			...
			///// MOUSE BUTTON PRESSED /////
			 if(mouse.pressed) {
				 if(activeP == null) {
					 ...
					 // If a player is holding a piece, simulate a move
					 simulate();
				 }
			 }
	...
	private boolean isCheckmate() { ////////to check if the king can move to a square where it is not under attack
		Piece king = getKing(true);
	...
	private boolean kingCanMove(Piece king) {
		...
		if(isValidMove(king, -1,-1)) return true;  //up left
		if(isValidMove(king, 0,-1)) return true;	//up
		if(isValidMove(king, 1,-1)) return true;	//down left
		if(isValidMove(king, -1,0)) return true;
		if(isValidMove(king, 1,0)) return true;
		if(isValidMove(king, -1,1)) return true;
		if(isValidMove(king, 0,1)) return true;
		if(isValidMove(king, 1,1)) return true;		//down right
